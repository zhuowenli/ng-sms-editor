  ...
  function smsEditorContent () {
    return {
      ...
      link: function link($scope, element, attr, ngModel) {
        ...
        var quill$$1 = new Quill(element[0], options); // 填充文本

        ngModel.$render = function () {
          var value = ngModel.$viewValue;

          if (value) {
            var length = quill$$1.getLength();
            var labels = []; // 短信变量
            ...
            value.replace(/#(?!\s*#)[^#]+#/g, function (text) {
              ...
              var length = quill$$1.getLength(); // 如果不是 ## 包裹的内容，直接插入文本
              ...
              if (/#(?!\s*#)[^#]+#/.test(text) && (!labels.length || labels.indexOf(text) >= 0)) {
                ...
                quill$$1.insertEmbed(length - 1, 'mark', {
                  value: text,
                  id: id
                }, Quill.sources.USER);
                quill$$1.insertText(length, ' ');

                if (inx === value.length - 1) {
                  quill$$1.insertText(length + 1, "\uFEFF");
                }
              } else {
                quill$$1.insertText(length - 1, text);
              }
            });
          }
        }; // 监听文本插入
        ...
        $scope.$watch('insertText', function () {
          ...
          if (value) {
            var range = quill$$1.getSelection(true);
            var id = parseInt(Math.random() * 1e8, 10);
            ...
            quill$$1.insertEmbed(range.index, 'mark', {
              value: value,
              id: id
            }, Quill.sources.USER);
            var length = quill$$1.getLength();
            ...
            quill$$1.insertText(range.index + 1, text);
            quill$$1.setSelection(range.index + 2, Quill.sources.SILENT);
            $scope.insertText = '';
          }
        }); // 监听文本改动

        quill$$1.on('text-change', function () {
          var _quill$getContents = quill$$1.getContents(),
        ...
        quill$$1.clipboard.addMatcher(Node.ELEMENT_NODE, function (node) {
          var innerText = node.innerText;
          var text = /^\s*c\.tb\.cn/.test(innerText) ? innerText : innerText.trim();
          return new delta().insert(text.replace(/\ud83c[\udf00-\udfff]|\ud83d[\udc00-\ude4f]|\ud83d[\ude80-\udeff]|\ud83e[\udd10-\udd80]/g, ''));
        });
        quill$$1.clipboard.addMatcher(Node.TEXT_NODE, function (node) {
          var data = node.data;
